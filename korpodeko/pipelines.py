...
class KorpodekoPipeline:

    # Database setup
    conn = sqlite3.connect('korpodeko.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `korpodeko`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM korpodeko WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO korpodeko (date, title, link, content)"
